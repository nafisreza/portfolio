...
const NavItem = ({ href, children, onClick, index, delay }: NavItemsProps) => {
  return (
    ...
    >
      <Link
        href={href || `/#${children}`}
        className="p-2 font-mono hover:text-accent duration-500 block"
        onClick={onClick}
        withPadding
      >
        {children}
...
const Navbar = () => {
  ...
  return (
    <motion.header
      ...
      animate="show"
      id="navbar"
      className="px-8 md:px-6 xl:px-12 py-6 fixed inset-x-0 top-0 right-0 flex justify-between items-center z-50 duration-500 backdrop-blur-lg"
    >
        ...
        >
          ...
          >
            {navLinks.map(({ name, url, index }, i) => (
              ...
              >
               <span className='text-accent'>{index}</span> {name}
              </NavItem>
            ))}
